...
dataset = load_dataset("Coder-Dragon/wikipedia-movies", split="train")
dataset = dataset.filter(lambda x: x['Release Year'] >= 2001)
dataset = dataset.remove_columns(["Wiki Page", "Image"])
...
for i, row in enumerate(subset):
    ...
    sentences = get_fixed_chunks(text)
    page_number = i + 1 
    ...
    for j, sentence in enumerate(sentences):
        ...
        chunk_number = j + 1 
        continuous_id = str(len(final_chunks) - 1)
        structural_marker = f"movie{page_number}_{chunk_number}"
        ...
        chunk_meta ={
            ...
            "Release_Year": year,
            "chunk_number": chunk_number,
            "global_index": structural_marker
        }
...
while True:
    ...
    for i in range(len(results['ids'][0])):
        ...
        year = metadata.get("Release_Year", "N/A")
        print(f"{i+1}. ID: {chunk_id} | Мета: Название фильма {title}, Чанк {chunk_val}, Жанр {genre}, Год {year} | Дистанция: {distance:.4f}")